    ...
    def create_content_slide(
        ...
        if image_url_or_path:
            ...
            if image_url_or_path.startswith('http://') or image_url_or_path.startswith('https://'):
                try:
                    ...
                    temp_file.close()
                    image_url_or_path = temp_file.name
                    
                    logger.debug(f"Downloaded image from URL to temp file: {temp_file.name}")
                except Exception as e:
                    logger.error(f"Error downloading image from URL: {e}")
                    image_url_or_path = None
            elif not os.path.exists(image_url_or_path):
                # Local path but doesn't exist
                image_url_or_path = None
        ...
        if image_url_or_path:
            ...
            try:
                ...
                logger.debug(f"Added image to slide: {image_url_or_path}")
            except Exception as e:
